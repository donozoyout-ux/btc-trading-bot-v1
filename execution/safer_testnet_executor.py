...
class SaferTestnetExecutor(TestnetExecutor):
    ...
    def _capture_context(self, snapshot: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "initial_stop": plan.get("stop_loss"),
            "tp1": plan.get("tp1"),
            "tp2": plan.get("tp2"),
    ...
    def process_snapshot(self, snapshot: Dict[str, Any], state) -> Optional[Dict[str, Any]]:
        ...
        self._entry_context["initial_size"] = risk.get("position_size_btc")
    ...
    def _replace_stop_safely(self, position: Dict[str, Any], new_stop: float) -> None:
        ...
        if new_id not in {row.get("algoId") for row in verified}:
        ...
        if new_id not in {row.get("algoId") for row in final}:
            raise ExecutionError("PROTECTION_REPLACEMENT_FAILED")
        new_order["role"] = "STOP"
    ...
    def manage_adaptive_position(self, snapshot: Dict[str, Any], state, position: Dict[str, Any]) -> Dict[str, Any]:
        ...
        initial_stop = self._safe_float(self._entry_context.get("initial_stop")) or current_stop
        initial_size = self._safe_float(self._entry_context.get("initial_size")) or abs(float(position.get("position_amt") or 0))
