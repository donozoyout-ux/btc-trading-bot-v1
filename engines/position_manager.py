...
class PositionManager:
    ...
    def evaluate(self, *, direction: TradeDirection, entry: float, initial_stop: float, current_stop: float,
                 ...
                 volume_support: bool = True, data_healthy: bool = True, candle_closed: bool = True,
        ...
        if risk <= 0 or not candle_closed or not data_healthy:
            return self._decision(PositionManagementState.NO_CHANGE, ["DATA_UNAVAILABLE_KEEP_EXISTING_PROTECTION"], 0, mfe_r, mae_r,
                                  True, True, True, momentum_support, volume_support, management_profile, target_replan_count, last_target_replan_at)
        ...
        structure_valid = not (structure == opposite_structure and (last_bos == opposite_bos or last_choch == opposite_choch))
        ...
        if size_increased:
            return self._decision(PositionManagementState.NO_CHANGE, ["POSITION_SIZE_INCREASE_DETECTED", "AVERAGING_DOWN_BLOCKED"], current_r, mfe_r, mae_r,
                                  False, structure_valid, regime_support, momentum_support, volume_support, management_profile, target_replan_count, last_target_replan_at)
        ...
        if stop_widened:
            return self._decision(PositionManagementState.PROTECT, ["STOP_WIDENING_DETECTED", "RESTORE_INITIAL_MAX_LOSS"], current_r, mfe_r, mae_r,
                                  True, structure_valid, regime_support, momentum_support, volume_support, management_profile, target_replan_count, last_target_replan_at,
        ...
        if not regime_support and not structure_valid and not momentum_support:
        ...
        if current_r < 0 and current_r > -1 and structure_valid and (regime_support or momentum_support):
            ...
            return self._decision(state, ["TEMPORARY_ADVERSE_MOVE", "THESIS_STILL_VALID", "STRUCTURE_INTACT"], current_r, mfe_r, mae_r,
                                  True, True, regime_support, momentum_support, volume_support, management_profile, target_replan_count, last_target_replan_at)
        ...
        if (self.target_replan_enabled and current_r >= self.target_replan_min_r and strong_regime and momentum_support and volume_support
            ...
            return self._decision(PositionManagementState.TARGET_REPLAN, ["STRONG_TREND_CONTINUATION", "CLOSED_5M_CONFIRMATION", "NEXT_STRUCTURE_AVAILABLE"], current_r, mfe_r, mae_r,
                                  True, True, True, True, True, management_profile, target_replan_count + 1, candle_timestamp,
        ...
        if current_r >= self.breakeven_min_r and structure_valid:
            ...
            if safe_stop != current_stop:
                return self._decision(state, [reason, "STOP_NEVER_WIDENS"], current_r, mfe_r, mae_r, True, True, regime_support,
                                      momentum_support, volume_support, management_profile, target_replan_count, last_target_replan_at,
                                      stop_action={"action": "TIGHTEN_STOP", "new_stop": safe_stop, "quantity_increase": 0})
        return self._decision(PositionManagementState.HOLD, ["THESIS_STILL_VALID", "NO_OBJECTIVE_MANAGEMENT_CHANGE"], current_r, mfe_r, mae_r,
                              True, structure_valid, regime_support, momentum_support, volume_support, management_profile, target_replan_count, last_target_replan_at)

    @staticmethod
    def _decision(state, reasons, current_r, mfe_r, mae_r, thesis, structure, regime, momentum, volume,
        ...
        return PositionManagementDecision(
            state=state, reason_codes=reasons, confidence="HIGH" if thesis and structure else "MEDIUM",
            ...
            volume_support=volume, target_action=target_action or {}, stop_action=stop_action or {},
